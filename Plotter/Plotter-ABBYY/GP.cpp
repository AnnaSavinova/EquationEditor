...
{
	recalculateOrigin(shift, 0, 0);
	calculateRelativePoints();
}

void GP::moveAlongY( double shift )
{
	recalculateOrigin(0, shift, 0);
	calculateRelativePoints();
}

void GP::moveAlongZ( double shift )
{
	recalculateOrigin(0, 0, shift);
	calculateRelativePoints();
}
...
void GP::recalculateOrigin(double xShift, double yShift, double zShift) {
	std::pair<double, double> x = getAxisVectorVisual(0);
	std::pair<double, double> y = getAxisVectorVisual(1);
	std::pair<double, double> z = getAxisVectorVisual(2);
